...
const PreOrderDetailScreen = ({ route, navigation }) => {
  route.params === undefined ? navigation.navigate('Tüm Ön Siparişler') : route.params.id;
  const [user] = useAuth();
  ...
  const getOrdersByCustomerId = async () => {
    await AxiosClient.get(`/api/dealer/view/${user.tms_id}`, {
      headers: {
        Authorization: `Bearer ${user.accessToken}`,
      },
    })
      .then(response => {
        ...
        setOrderHolderNameAndSurname(response.data.orderer_name);
        setOrderHolderPhoneNumber(response.data.orderer_phone);
        setOrderHolderEmailAdress(response.data.orderer_email);
        setOrderWeight(JSON.stringify(response.data.weight));
        setCityValue(response.data.city_id);
        setDistrictValue(response.data.district_id);
        setOrderHolderAddress(response.data.address_description);
  ...
  const handleSavePreorder = async () => {
    ...
    await AxiosClient.post(
      ...
      .then(async function (response) {
        Alert.alert('Uyarı', 'Ön sipariş güncellendi', [{ text: 'Tamam' }]);
        navigation.navigate('Tüm Ön Siparişler');
      })
      .catch(function (error) {
        console.error(error.message);
        Alert.alert('Uyarı', 'Ön sipariş güncellenemedi', [{ text: 'Tamam' }]);
      })
      .finally(() => {});
  };
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
      <ScrollView
        ...
        <CustomizeButton insideText={'Kaydet'} onPressHandle={handleSavePreorder} />
      </ScrollView>
    </KeyboardAvoidingView>
  );
};
